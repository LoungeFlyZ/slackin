...
function splash(_ref) {
  ...
  var div = dom(".splash", !iframe && dom(".logos", logo && dom(".logo.org"), dom(".logo.slack")), dom("p", "Join ", dom("b", name),
  // mention single single-channel inline
  channels && channels.length === 1 && dom("span", " #", channels[0]), " on Slack."), dom("p.status", active ? [dom("b.active", active), " users online now of ", dom("b.total", total), " registered."] : [dom("b.total", total), " users are registered so far."]), dom("form",
  // channel selection when there are multiple
  channels && channels.length > 1 && dom("select.form-item name=channel", channels.map(function (channel) {
    return dom("option", { value: channel, text: channel });
  })), dom("input.form-item type=email placeholder=you@yourdomain.com " + (!iframe ? "autofocus" : "")), dom("button.loading", "Get my Invite")), !iframe && dom("p.signin", "or ", dom("a href=https://" + org + ".slack.com target=_top", "sign in"), "."), !iframe && dom("footer", "powered by ", dom("a href=http://rauchg.com/slackin target=_blank", "slackin")), style({ logo: logo, active: active, iframe: iframe }),
  // xxx: single build
  dom("script src=https://cdn.socket.io/socket.io-1.3.2.js"), dom("script src=assets/superagent.js"), dom("script src=assets/client.js"));
  return div;
}
